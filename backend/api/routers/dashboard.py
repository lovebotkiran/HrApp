...
async def get_pipeline_stats(
    ...
    stats = {
        ...
        "selected": db.query(Application).filter(Application.status == "hired").count(), # Using 'hired' as per model comments
...
async def get_recruitment_metrics(
    ...
    from infrastructure.database.models import Offer, JobPosting, Interview
    ...
    source_stats = {source: count for source, count in sources if source is not None}
